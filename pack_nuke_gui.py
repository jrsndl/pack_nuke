...
class MainWindow(QtWidgets.QMainWindow, Ui_pack_gui):
    ...
    def match_regex(self, filter, source, ):
        """
        ...
        try:
            ...
            return None
        if filter['replace'] and filter['replace'] != '':
            try:
                result = _re.match(source).expand(filter['replace'])
            except:
                return None
        else:
            m = re.search(filter['search'], source)
            if m is not None:
                try:
                    result = m.group(1)
                except:
    ...
    def get_script_path_tags(self, full_path):
        ...
        for one_filter in tags_list:
            ...
            if one_filter['source'] == 'File Name':
                match = self.match_regex(one_filter, source=file_name)
            elif one_filter['source'] == 'File Path':
                match = self.match_regex(one_filter, source=full_path)
    ...
    def _fill_job_path(self):
        ...
        # job root
        self.ui.job_folder.setText(self.settings_json['job']['job_root'].format_map(Default(self.anatomy)).replace("\\", "/"))
        # job folder
        self.ui.job_name.setText(self.settings_json['job']['job_name_default'].format_map(Default(self.anatomy)).replace("\\", "/"))
    ...
    def display_json(self):
        ...
        if self.settings_json is not None and self.settings_json != {}:
            ...
            if len(self.place_list) >= 2:
                ...
                logger.error(f"Found {len(self.place_list)} places defined in settings json. At least two are required.")
    ...
    def go(self):
        ...
        if not r:
            logging.error(f"Can't create directory {_job_path}")
        pack_dir = _job_path + '/_pack_nuke'
        r = make_dir(pack_dir)
        if not r:
            ...
            for one in self.filtered_versions:
                if one['apply']:
                    one_row = [
                        ...
                        one['full'],
                        one['tokens'],
                        self.ui.job_name.displayText(),
                        self.ui.place_source.currentText(),
